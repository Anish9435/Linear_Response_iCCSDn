...
def Sv_diagram_vs_contraction(Sv,II_vv):
  ...
  R_iuab += 2*np.einsum('idak,kudb->iuab',twoelecint_mo[:occ,occ:nao,occ:nao,:occ],Sv) ####WHERE X IS Sv OF FIRST OR ZEROTH ORDER####
...
def Sv_diagram_vt_contraction(t2):
  ...
  R_iuab += -np.einsum('dukb,kiad->iuab',twoelecint_mo[occ:nao,occ:occ+v_act,:occ,occ:nao],t2)  ####WHERE X IS t2 OF FIRST OR ZEROTH ORDER####
...
def So_diagram_vs_contraction(So,II_oo):
  ...
  R_ijav += -np.einsum('dila,jldv->ijav',twoelecint_mo[occ:nao,:occ,:occ,occ:nao],So)      ####WHERE X IS So OF FIRST OR ZEROTH ORDER####
